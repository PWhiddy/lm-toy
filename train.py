...
def main():
  ...
  model = Model(vocab_size, context_size=context_size, hidden_dim=hidden_dim, layers=2)
  ...
  with torch.no_grad(): 
    init_inp, init_out = get_batch(test_corpus, token_map, batch_size, context_size)
  ...
  for i in range(iterations):
    ...
    for e in range(steps_per_iter):
      input_t, output_t = get_batch(train_corpus, token_map, batch_size, context_size)
      opt.zero_grad()
    ...
    print(f"mean iteration train loss: {mean_train_loss}")
    eval_in, eval_out = get_batch(test_corpus, token_map, batch_size, context_size)
    with torch.no_grad():
...
def get_batch(data, token_map, batch_size, context_size):
  max_idx = len(data) - context_size - 1
  offsets = [random.randint(0, max_idx) for _ in range(batch_size)]
  input_tokens = torch.tensor([[token_map[c] for c in data[off:off+context_size]] for off in offsets])
  #print(f"input chars : {input_tokens}")
  output_tokens = torch.tensor([token_map[data[off+context_size]] for off in offsets])
